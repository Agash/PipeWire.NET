...
public sealed class ExplicitSyncTests : PipeWireTestBase
{
    ...
    public async Task SyncProducer_ToSyncConsumer_CarriesPointsAndCompletesTheHandshake()
    {
        ...
        try
        {
            ...
            output.AllocateDmaBufSync += (_, index, w, h, _, _, planes, out acquireFd, out releaseFd) =>
            {
    ...
    {
        if (!File.Exists("/dev/dri/renderD128"))
            Assert.Inconclusive("No GPU render node (/dev/dri/renderD128) - explicit sync needs one.");
        ...
        const int width = 320, height = 240, poolCap = 16;
        GbmAllocator gbm;
        try
        {
            gbm = new GbmAllocator("/dev/dri/renderD128");
        ...
        try
        {
            ...
            await using var producerCtx = new PipeWireContext("pwnet-gate-producer", ConsoleTestLoggerFactory.Instance);
            await producerCtx.StartAsync(cts.Token);
            await using var consumerCtx = new PipeWireContext("pwnet-gate-consumer", ConsoleTestLoggerFactory.Instance);
            await consumerCtx.StartAsync(cts.Token);

            long modifier = (long)GbmAllocator.LinearModifier;
            long stamped = 0;

            await using var output = new PipeWireVideoOutput(producerCtx, "pwnet-gate", width, height, PixelFormat.Bgra, 30);
            output.AllocateDmaBufSync += (_, index, _, _, _, _, planes, out acquireFd, out releaseFd) =>
            {
                acquireFd = -1;
                releaseFd = -1;
                if (index >= poolCap) return 0;
            ...
            capture.FrameReady += (_, frame) =>
            {
                if (frame.SyncTimeline is null || frame.Planes.IsEmpty) return;
                lock (deliveries) deliveries.Add((frame.Planes[0].Fd, DateTime.UtcNow.Ticks));
            };
            capture.Connect(nodeId, [PixelFormat.Bgra], modifiers: [modifier], requestExplicitSync: true);
            await capture.WaitForStreamingAsync(cts.Token);

            // Let the pool turn over so every buffer has been seen at least once.
            await Task.Delay(1000, cts.Token);

            Assert.IsTrue(capture.TryGetBorrowedFrame(out BorrowedVideoFrame held), "no borrowed frame to hold");
            ...
            lock (deliveries) seen = [.. deliveries];

            Assert.IsTrue(seen.Count(d => d.AtTicks < heldAt) >= poolCap,
                "the pool never turned over before the hold, so there is nothing to compare against");

            int reusedWhileHeld = seen.Count(d => d.Fd == heldFd && d.AtTicks > heldAt && d.AtTicks < releasedAt);
            Assert.AreEqual(0, reusedWhileHeld,
                "the producer wrote into a buffer the consumer was still holding - the release point did not gate reuse");

            Assert.IsTrue(seen.Any(d => d.Fd == heldFd && d.AtTicks > releasedAt),
    ...
    public async Task AnAppSuppliedAcquireTimeline_IsLeftForTheAppToSignal()
    {
        ...
        if (!DrmSyncobj.IsAvailable)
            Assert.Inconclusive("No render node with timeline syncobjs, so the app has no timelines to supply.");
        ...
        var buffers = new List<GbmAllocator.Buffer>();
        var timelines = new (uint AcquireHandle, int AcquireFd, uint ReleaseHandle, int ReleaseFd)[poolCap];
        ...
        try
        {
            ...
            await using var producerCtx = new PipeWireContext("pwnet-appsync-producer", ConsoleTestLoggerFactory.Instance);
            await producerCtx.StartAsync(cts.Token);
            await using var consumerCtx = new PipeWireContext("pwnet-appsync-consumer", ConsoleTestLoggerFactory.Instance);
            await consumerCtx.StartAsync(cts.Token);

            long modifier = (long)GbmAllocator.LinearModifier;
            long stamped = 0;

            var output = new PipeWireVideoOutput(producerCtx, "pwnet-appsync", width, height, PixelFormat.Bgra, 30);
            try
            {
                output.AllocateDmaBufSync += (_, index, _, _, _, _, planes, out acquireFd, out releaseFd) =>
                {
                ...
                await using var capture = new PipeWireVideoCapture(consumerCtx, "pwnet-appsync-sink");
                capture.FrameReady += (_, frame) =>
                {
                    if (frame.SyncTimeline is not { AcquirePoint: > 0 } timeline) return;
                    lock (arrivals) arrivals.Add((timeline.AcquirePoint, DateTime.UtcNow.Ticks));
                };
                capture.Connect(nodeId, [PixelFormat.Bgra], modifiers: [modifier], requestExplicitSync: true);
                await capture.WaitForStreamingAsync(cts.Token);
