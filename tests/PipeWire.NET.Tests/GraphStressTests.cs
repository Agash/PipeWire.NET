...
public sealed class GraphStressTests : PipeWireTestBase
{
    ...
    private static int OpenFileDescriptors()
    {
        int ours = 0;
        foreach (
            string entry in Directory.GetFileSystemEntries($"/proc/{Environment.ProcessId}/fd")
        )
        {
            string? target;
            try
    ...
    public async Task RepeatedCreateAndDestroy_LeaksNeitherDescriptorsNorMemory()
    {
        ...
        await using (registry)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                PipeWireNode warm = await registry.CreateVirtualSinkAsync(
                    $"W{i}",
                    $"pwnet_w{i}",
                    cts.Token
                );
    ...
    public async Task ConcurrentCreates_AllSucceedAndAllReachTheGraph()
    {
        ...
        await using (registry)
        {
            ...
            [
                .. Enumerable
                    ...
                    .Select(i =>
                        Task.Run(
                            () =>
                                registry.CreateVirtualSinkAsync(
                                    $"P{i}",
                                    $"pwnet_par_{i}",
                                    cts.Token
                                ),
            ...
            Assert.AreEqual(
                Degree,
                nodes.Select(n => n.NodeId).Distinct().Count(),
            ...
            foreach (PipeWireNode node in nodes)
                Assert.IsNotNull(
                    graph.GetNode(node.NodeId),
                    $"node {node.NodeId} never reached the graph"
                );
        }
    }
        ...
        await using (context)
        {
            ...
            [
                .. Enumerable
                    .Range(0, 8)
                    .Select(i =>
                        registry.CreateVirtualSinkAsync($"T{i}", $"pwnet_tear_{i}", cts.Token)
                    ),
            ];
            ...
            foreach (Task<PipeWireNode> create in creates)
            {
                try
                {
                    ...
                    Assert.AreNotEqual(
                        0u,
                        node.NodeId,
                        "a completed creation must yield a real node"
                    );
                    completed++;
                }
                catch (ObjectDisposedException)
                {
                    refusals.Add(nameof(ObjectDisposedException));
                }
                catch (OperationCanceledException)
                {
                    refusals.Add(nameof(OperationCanceledException));
                }
            }

            Assert.AreEqual(
                creates.Length,
                completed + refusals.Count,
    ...
    public async Task ASnapshotHeldAcrossHeavyChurn_StaysInternallyConsistent()
    {
        ...
        await using (registry)
        {
            ...
            PipeWireGraphSnapshot held = await WaitForAsync(
                registry,
                g => g.GetPortsForNode(a.NodeId).Length == 4,
                cts.Token
            );
            ...
            for (int i = 0; i < 15; i++)
            {
                PipeWireNode churn = await registry.CreateVirtualSinkAsync(
                    $"CC{i}",
                    $"pwnet_cc_{i}",
                    cts.Token
                );
            ...
            foreach (PipeWirePort port in held.Ports)
                Assert.IsNotNull(
                    held.GetPort(port.PortId),
                    $"port {port.PortId} is in Ports but not the index"
                );
    ...
    public async Task RemovingAnIdThatNeverExisted_IsReportedAsAFailure()
    {
        ...
        await using (registry)
        {
            await Assert.ThrowsAsync<PipeWireException>(() =>
                registry.DestroyGlobalAsync(999_999, cts.Token)
            );
        }
    }
    ...
    public async Task RemovingAnObjectTwice_IsRefusedRatherThanFatal()
    {
        ...
        await using (registry)
        {
            ...
            await Assert.ThrowsAsync<PipeWireException>(() =>
                registry.DestroyGlobalAsync(node.NodeId, cts.Token)
            );
        }
    }
}
