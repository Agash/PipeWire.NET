...
public sealed class GraphLifetimeTests : PipeWireTestBase
{
        ...
        await using (observer)
        {
            ...
            await using (ownerContext)
            {
                ...
                PipeWireGraphSnapshot seen = await WaitForAsync(
                    observer,
                    g => g.GetPortsForNode(nodeId).Length == 4,
                    cts.Token
                );
            ...
            PipeWireGraphSnapshot after = await WaitForAsync(
                observer,
                g => g.GetNode(nodeId) is null,
                cts.Token
            );

            Assert.AreEqual(
                0,
                after.GetPortsForNode(nodeId).Length,
    ...
    public async Task DisposingTheOwningRegistryTwice_DestroysEachProxyOnce()
    {
        ...
        await using (context)
        {
            ...
            (PipeWireContext watcherContext, PipeWireRegistry watcher) = await ConnectAsync(
                "pwnet-dispose-watch",
                cts.Token
            );
    ...
    public async Task ADestroyedNodesProxyIsReleasedByTheRemovalPath()
    {
        ...
        await using (registry)
        {
            ...
            Assert.IsNotNull(
                registry.Current.GetLink(link.LinkId),
                "the link must exist before removal"
            );
            ...
            Assert.AreEqual(
                0,
                after.GetOutputLinksForPort(link.OutputPortId).Length,
    ...
    public async Task ACreatedNodeCarriesUsablePermissions()
    {
        ...
        await using (registry)
        {
            ...
            foreach (PipeWirePort port in graph.GetPortsForNode(node.NodeId))
                Assert.IsTrue(
                    port.Permissions.HasFlag(PipeWirePermissions.Read),
    ...
    public async Task EverythingBuiltDeliberatelyLeftBehind_CanStillBeTornDown()
    {
        ...
        await using (registry)
        {
            PipeWireNode a = await registry
                .CreateVirtualSink("Cleanup A")
                .WithName(tag + "_a")
                .WithLinger()
                .ExecuteAsync(cts.Token);
            PipeWireNode b = await registry
                .CreateVirtualSink("Cleanup B")
                .WithName(tag + "_b")
                .WithLinger()
                .ExecuteAsync(cts.Token);

            PipeWirePort output = await WaitForPortAsync(
                registry,
                a.NodeId,
                PipeWirePortDirection.Out,
                cts.Token
            );
            PipeWirePort input = await WaitForPortAsync(
                registry,
                b.NodeId,
            ...
            using PipeWireDeviceProvider device = PipeWireDeviceProvider.Create(
                context,
                tag + "_device",
                "A device this test withdraws"
            );
            using PipeWireMetadataProvider served = PipeWireMetadataProvider.Create(
                context,
                tag + "_meta"
            );
            ...
            await using (store)
            {
                ...
                Assert.IsNotNull(
                    built.Devices.FirstOrDefault(d => d.DeviceName == tag + "_device")
                );
    ...
    private static bool LeftoversPresent(PipeWireGraphSnapshot graph, string tag) =>
        graph.Nodes.Any(n =>
            n.NodeName is not null && n.NodeName.Contains(tag, StringComparison.Ordinal)
        )
        || graph.Devices.Any(d =>
            d.DeviceName is not null && d.DeviceName.Contains(tag, StringComparison.Ordinal)
        )
        || graph.Objects.Any(o =>
