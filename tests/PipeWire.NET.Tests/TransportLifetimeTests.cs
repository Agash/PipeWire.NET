...
public sealed class TransportLifetimeTests : PipeWireTestBase
{
    ...
    public async Task EveryBufferTheAppBacked_IsReleasedBackToIt()
    {
        ...
        {
            await using (
                var ctx = new PipeWireContext("pwnet-release", ConsoleTestLoggerFactory.Instance)
            )
            {
                ...
                Assert.IsTrue(
                    ...
                    "no frames flowed, so no pool was built"
                );
    ...
    public async Task EachTriggeredPublish_ReachesTheConsumerWithItsContent()
    {
        ...
        output.FillFrame += (_, pixels, stride, width, height, _) =>
        {
            ...
            for (var y = 0; y < height; y++)
                pixels.Slice(y * stride, width * 4).Fill(tag);
            return true;
        };
        ...
        for (var i = 0; i < 100 && !output.IsDriving; i++)
            await Task.Delay(50, cts.Token);
        Assert.IsTrue(
            output.IsDriving,
            "the daemon never made the driver-flagged output the graph's driver"
        );
        ...
        for (var i = 0; i < triggers; i++)
        {
            ...
            int seenBefore;
            lock (tags)
                seenBefore = tags.Count;
            try
            {
                await output
                    .TriggerProcessAndWaitAsync(cts.Token)
        ...
        Assert.IsTrue(seen.Length > 0, "the triggered publishes never reached the consumer");
