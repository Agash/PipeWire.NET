...
public sealed class ThirdPartyGraphTests : PipeWireTestBase
{
    ...
    public async Task NodesWeCreate_AreVisibleToPwLinkWithTheSameIds()
    {
        ...
        await using (reg)
        {
            PipeWireNode node = await reg.CreateVirtualSink("Visible")
                .WithName("pwnet_tp_visible")
                .ExecuteAsync(cts.Token);
            PipeWireGraphSnapshot graph = await WaitForPortsAsync(reg, node.NodeId, cts.Token);

            Dictionary<string, uint> outputs = await PwTools.ListPortsAsync(
                outputs: true,
                cts.Token
            );
            Dictionary<string, uint> inputs = await PwTools.ListPortsAsync(
                outputs: false,
                cts.Token
            );
            ...
            foreach (PipeWirePort port in graph.GetPortsForNode(node.NodeId))
            {
                ...
                Assert.IsTrue(
                    side.TryGetValue(full, out uint theirId),
    ...
    public async Task ALinkWeCreate_IsListedByPwLinkWithMatchingEndpoints()
    {
        ...
        await using (reg)
        {
            (PipeWireNode a, PipeWireNode b, PipeWireGraphSnapshot ready) = await TwoNodesAsync(
                reg,
                "pwnet_tp_la",
                "pwnet_tp_lb",
                cts.Token
            ...
            List<(uint Link, uint Output, uint Input)> theirs = await PwTools.ListLinksAsync(
                cts.Token
            );
            (uint Link, uint Output, uint Input) match = theirs.FirstOrDefault(l =>
                l.Link == link.LinkId
            );
            ...
            Assert.AreEqual(
                output.PortId,
                match.Output,
                "we disagree on which port the link leaves"
            );
    ...
    public async Task ALinkPwLinkCreates_ReachesOurSnapshotWithTheRightEndpoints()
    {
        ...
        await using (reg)
        {
            ...
            await PwTools.LinkAsync(
                $"pwnet_tp_ta:{output.PortName}",
                $"pwnet_tp_tb:{input.PortName}",
                cts.Token
            );

            PipeWireGraphSnapshot linked = await WaitForAsync(
                reg,
                g => g.GetOutputLinksForPort(output.PortId).Length == 1,
                cts.Token
            );
    ...
    public async Task ALinkPwLinkRemoves_LeavesOurSnapshot()
    {
        ...
        await using (reg)
        {
            ...
            PipeWireGraphSnapshot after = await WaitForAsync(
                reg,
                g => g.GetLink(link.LinkId) is null,
                cts.Token
            );
    ...
    public async Task AMidiNode_ReportsNoChannelMap()
    {
        ...
        await using (reg)
        {
            ...
            PipeWireGraphSnapshot seen = await WaitForAsync(
                reg,
                g => g.GetNode(filterNode) is not null,
                cts.Token
            );
            ...
            Assert.AreEqual(
                "Midi/Sink",
                midi.MediaClass,
                "the filter did not publish as a MIDI node"
            );
    ...
    public async Task AThirdPartyNodeAppearsAndWeCanLinkToIt()
    {
        ...
        await using (reg)
        {
            ...
            PipeWireGraphSnapshot graph = await WaitForAsync(
                ...
                g =>
                    g.Nodes.Any(n =>
                        n.NodeName == "input.pwnet_tp_loop"
            ...
            PipeWireNode theirOutputSide = graph.Nodes.First(n =>
                n.NodeName == "output.pwnet_tp_loop"
            );
            ...
            PipeWirePort theirInput = graph
                .GetPortsForNode(theirs.NodeId, PipeWirePortDirection.In)
                .OrderBy(p => p.PortId)
                .First();

            PipeWireNode ours = await reg.CreateVirtualSink("Feeder")
                .WithName("pwnet_tp_feeder")
                .ExecuteAsync(cts.Token);
            PipeWireGraphSnapshot ready = await WaitForPortsAsync(reg, ours.NodeId, cts.Token);
            PipeWirePort ourOutput = ready
                .GetPortsForNode(ours.NodeId, PipeWirePortDirection.Out)
                .OrderBy(p => p.PortId)
                .First();
            ...
            Assert.AreEqual(
                theirs.NodeId,
                reg.Current.GetLink(link.LinkId)!.InputNodeId,
        ...
        await using (reg)
        {
            ...
            uint theirNode;
            {
                ...
                PipeWireGraphSnapshot graph = await WaitForAsync(
                    ...
                    g =>
                        g.Nodes.Any(n =>
                            n.NodeName == "input.pwnet_tp_vanish"
                ...
                Assert.IsTrue(
                    graph.GetPortsForNode(theirNode).Length > 0,
            ...
            PipeWireGraphSnapshot after = await WaitForAsync(
                reg,
                g => g.GetNode(theirNode) is null,
                cts.Token
            );

            Assert.AreEqual(
                0,
                after.GetPortsForNode(theirNode).Length,
    ...
    public async Task AThirdPartyChurningLinks_KeepsOurSnapshotConsistent()
    {
        ...
        await using (reg)
        {
                        ...
                        while (!stop.IsCancellationRequested)
                        {
                            ...
                            foreach (PipeWireLink link in g.Links)
                            {
                                ...
                                if (!g.GetInputLinksForPort(link.InputPortId).Contains(link))
                                    throw new InvalidOperationException(
                                        $"link {link.LinkId} missing from the input index"
                                    );
                            }
            ...
            {
                await PwTools.LinkAsync(
                    $"pwnet_tp_ca:{output.PortName}",
                    $"pwnet_tp_cb:{input.PortName}",
                    cts.Token
                );
                ...
                await PwTools.DisconnectAsync(
                    on.GetOutputLinksForPort(output.PortId).Single().LinkId,
                    cts.Token
                );
                await WaitForAsync(
                    reg,
                    g => g.GetOutputLinksForPort(output.PortId).Length == 0,
                    cts.Token
                );
            }
            ...
            Assert.IsNull(
                torn,
                $"a reader saw an inconsistent graph while a third party edited it: {torn}"
            );
    ...
    public async Task OurViewOfLinks_MatchesPwLinkExactlyAfterChurn()
    {
        ...
        await using (reg)
        {
            ...
            PipeWirePort[] inputs =
            [
                .. ready.GetPortsForNode(b.NodeId, PipeWirePortDirection.In).OrderBy(p => p.PortId),
            ];

            // Wire both channels, one by each side, then compare the whole picture.
            PipeWireLink ourLink = await reg.CreateLink(outputs[0], inputs[0])
                .ExecuteAsync(cts.Token);
            await PwTools.LinkAsync(
                $"pwnet_tp_aa:{outputs[1].PortName}",
                $"pwnet_tp_ab:{inputs[1].PortName}",
                cts.Token
            );
            ...
            PipeWireGraphSnapshot graph = await WaitForAsync(
                reg,
                g =>
                    g.GetOutputLinksForPort(outputs[0].PortId).Length == 1
            ...
            Assert.IsNotNull(
                graph.GetLink(ourLink.LinkId),
                "our own link must still be there among theirs"
            );
        }
    }
    ...
    private static async Task<(
        PipeWireNode A,
        PipeWireNode B,
        PipeWireGraphSnapshot Ready
    )> TwoNodesAsync(PipeWireRegistry reg, string nameA, string nameB, CancellationToken ct)
    {
