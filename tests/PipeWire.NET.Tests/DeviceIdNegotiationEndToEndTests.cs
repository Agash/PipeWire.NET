...
public sealed class DeviceIdNegotiationEndToEndTests : PipeWireTestBase
{
    ...
    private sealed record Outcome(
        ...
        int DmaBufFrames,
        int SyncFrames,
        DrmDevice? OutputDevice,
        DrmDevice? CaptureDevice,
    ...
    private static (DrmDevice Device, GbmAllocator Gbm) RealDevice()
    {
        ...
        {
            Assert.Inconclusive(
                $"libgbm unavailable ({ex.Message}) - skipping device-ID negotiation."
            );
            throw;
        }
    }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                output.AllocateDmaBuf += (_, index, _, _, _, device, planes) =>
                    Back(index, device, planes);
            ...
            capture.FrameReady += (_, frame) =>
            {
                ...
                if (frame.SyncTimeline is not null)
                    Interlocked.Increment(ref syncFrames);
            };
            ...
            return new Outcome(
                ...
                Volatile.Read(ref dmaBufFrames),
                Volatile.Read(ref syncFrames),
                output.NegotiatedDevice,
                capture.NegotiatedDevice,
                allocated
            );
        }
        finally
    ...
    public async Task BothEndsNegotiating_SettleOnTheDeviceTheyShare_AndTheAllocatorIsHandedIt()
    {
        ...
        using (gbm)
        {
            ...
            Assert.IsTrue(
                o.DmaBufFrames >= 10,
                $"expected DMA-BUF frames, got {o.DmaBufFrames} of {o.Frames}"
            );
            ...
            Assert.AreEqual(
                real.RenderNodePath,
                o.OutputDevice!.Value.RenderNodePath,
            ...
            Assert.IsTrue(
                o.HandedToAllocator.All(d => d == real),
    ...
    public async Task AConsumerThatDoesNotNegotiate_StillStreams_WithTheDeviceUndefined()
    {
        ...
        using (gbm)
        {
            ...
            Assert.IsNull(
                o.OutputDevice,
                "no device can have been negotiated with a peer that does not negotiate"
            );
            Assert.IsNull(o.CaptureDevice);
            Assert.IsFalse(o.HandedToAllocator.IsEmpty, "the allocator was never called");
            Assert.IsTrue(
                o.HandedToAllocator.All(d => d is null),
                "the allocator is told the device is undefined"
            );
        }
    }
    ...
    public async Task ExplicitSync_OverANegotiatedDevice_CarriesTimelinesAndTheDevice()
    {
        ...
        if (!DrmSyncobj.IsAvailable)
            Assert.Inconclusive("No render node supports syncobj timelines.");
        ...
        using (gbm)
        {
            ...
            Assert.IsTrue(
                o.SyncFrames >= 10,
                $"expected frames carrying timelines, got {o.SyncFrames} of {o.Frames}"
            );
    ...
    public async Task EndsWithNoDeviceInCommon_DoNotSettleOnOne()
    {
        ...
        using (gbm)
        {
            ...
            Assert.IsNull(o.CaptureDevice);
            Assert.IsTrue(
                o.HandedToAllocator.IsEmpty,
                "nothing was negotiated, so nothing should be allocated"
            );
        }
    }
}
