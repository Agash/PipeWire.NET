    ...
    public async Task TheFullTransportLoop_PreservesContentAndAlignment()
    {
        ...
        try
        {
            ...
            await using var originVideo = new PipeWireVideoOutput(
                ctx,
                $"pwnet-loop-v0-{Environment.ProcessId}",
                Width,
                Height,
                PixelFormat.Bgra,
                30
            ...
            originVideo.AllocateDmaBuf += (_, index, _, _, _, _, planes) =>
            {
                if (index >= PoolCap)
                    return 0;
                while (originBuffers.Count <= index)
                    originBuffers.Add(gbm.CreateBgra(Width, Height));
            ...
            await using var originAudio = new PipeWireAudioOutput(
                ctx,
                $"pwnet-loop-a0-{Environment.ProcessId}",
                Rate,
                1,
            ...
            {
                if (f.PresentationTimestampNs is { } pts)
                    Volatile.Write(ref relayPts, pts);
            };
            ...
            {
                if (relayAudio.Count > 40000)
                    return;
                foreach (float v in MemoryMarshal.Cast<byte, float>(f.Samples))
                    relayAudio.Enqueue(v);
            };

            relayAudioIn.Connect(
                (await originAudio.WaitForNodeIdAsync(cts.Token)),
                sampleRate: Rate,
                channels: 1,
            ...
            await using var relayVideoOut = new PipeWireVideoOutput(
                ctx,
                $"pwnet-loop-v1-{Environment.ProcessId}",
                Width,
                Height,
                PixelFormat.Bgra,
                30
            ...
            relayVideoOut.AllocateDmaBuf += (_, index, _, _, _, _, planes) =>
            {
                if (index >= PoolCap)
                    return 0;
                while (relayBuffers.Count <= index)
                    relayBuffers.Add(gbm.CreateBgra(Width, Height));
            ...
            await using var relayAudioOut = new PipeWireAudioOutput(
                ctx,
                $"pwnet-loop-a1-{Environment.ProcessId}",
                Rate,
                1,
                AudioSampleFormat.F32Le
            );
            ...
            finalVideo.FrameReady += (_, f) =>
            {
                if (f.BufferType == PipeWireBufferType.DmaBuf)
                    Interlocked.Increment(ref dmaBufOut);
            ...
            finalAudio.Connect(
                (await relayAudioOut.WaitForNodeIdAsync(cts.Token)),
                sampleRate: Rate,
                channels: 1,
                format: AudioSampleFormat.F32Le
            );
            ...
            Assert.IsTrue(
                measured.Length >= 3,
                $"only {measured.Length} frames completed the loop"
            );
            ...
            int start = 0;
            while (start < heard.Length && heard[start] == 0f)
                start++;
        ...
        {
            foreach (GbmAllocator.Buffer b in originBuffers)
                b.Dispose();
            foreach (GbmAllocator.Buffer b in relayBuffers)
                b.Dispose();
        }
    }
}
