...
public sealed partial class PipeWireDeviceProxy : PipeWireParameterObject
{
    private readonly ILogger _logger;

    private PipeWireDeviceProxy(PipeWireContext ctx, uint id, ILogger logger)
    ...
    private static unsafe void OnInfoCallback(void* data, pw_device_info* info)
    {
        ...
        {
            if (info is not null)
                if (FromUserData<PipeWireDeviceProxy>(data) is { } self)
                {
    ...
    )
    {
        try
        {
            FromUserData<PipeWireDeviceProxy>(data)?.OnParam(seq, param);
        }
        catch
        {
            // Deliberately not logged: the instance the logger belongs to is what failed to resolve.
