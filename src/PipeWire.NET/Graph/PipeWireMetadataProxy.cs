...
public sealed partial class PipeWireMetadataProxy : IDisposable, IAsyncDisposable
{
    ...
    private PipeWireMetadataProxy(PipeWireContext ctx, uint id, ILogger logger)
    {
        _ctx = ctx;
        Id = id;
        _logger = logger;
    ...
    private unsafe int ClearNative()
    {
        ...
        if (!_bound!.TryUse(out BoundProxy.Use proxy))
            throw new ObjectDisposedException(nameof(PipeWireMetadataProxy));
