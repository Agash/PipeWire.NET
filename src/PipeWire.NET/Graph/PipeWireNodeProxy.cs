...
public sealed partial class PipeWireNodeProxy : PipeWireParameterObject
{
    private readonly ILogger _logger;

    private PipeWireNodeProxy(PipeWireContext ctx, uint id, ILogger logger)
    ...
    private static unsafe void OnInfoCallback(void* data, pw_node_info* info)
    {
        ...
        {
            if (info is not null)
                if (FromUserData<PipeWireNodeProxy>(data) is { } self)
                {
    ...
    )
    {
        ...
        try
        {
            FromUserData<PipeWireNodeProxy>(data)?.OnParam(seq, param);
        }
        catch
        {
            // Deliberately not logged: the instance the logger belongs to is what failed to resolve,
