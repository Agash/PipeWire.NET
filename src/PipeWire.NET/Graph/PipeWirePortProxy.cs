...
public sealed partial class PipeWirePortProxy : PipeWireParameterObject
{
    private readonly ILogger _logger;

    private PipeWirePortProxy(PipeWireContext ctx, uint id, ILogger logger)
    ...
    private static unsafe void OnInfoCallback(void* data, pw_port_info* info)
    {
        ...
        {
            if (info is not null)
                if (FromUserData<PipeWirePortProxy>(data) is { } self)
                {
        ...
        void* data, int seq, uint id, uint index, uint next, spa_pod* param)
    {
        try
        {
            FromUserData<PipeWirePortProxy>(data)?.OnParam(seq, param);
        }
        catch
        {
            // Deliberately not logged: the instance the logger belongs to is what failed to resolve.
