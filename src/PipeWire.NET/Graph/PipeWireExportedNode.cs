...
public sealed unsafe partial class PipeWireExportedNode : IAsyncDisposable
{
    ...
    /// </remarks>
    public delegate int ProcessHandler(PipeWireExportedNode node, Span<byte> data);
    ...
    private PipeWireExportedNode(PipeWireContext ctx, string name, SpaDirection direction)
    {
        _ctx = ctx;
        _name = name;
        _direction = direction;
        _logger = ctx.LoggerFactory.CreateLogger<PipeWireExportedNode>();
    }
    ...
    public static PipeWireExportedNode Export(
        PipeWireContext ctx,
        string name,
        ...
        IReadOnlyDictionary<string, string>? properties = null)
    {
        ...
        var node = new PipeWireExportedNode(ctx, name, direction) { NegotiatedFormat = null };
    ...
    /// </exception>
    public static PipeWireExportedNode ExportSpaFactory(
        PipeWireContext ctx,
        string factoryName,
        IReadOnlyDictionary<string, string>? properties = null,
        string? libraryName = null)
    {
        ...
        var node = new PipeWireExportedNode(ctx, factoryName, SpaDirection.Output);
        node.InitializeFromFactory(factoryName, properties, asDevice: false, libraryName);
        return node;
    }

    /// <summary>
    /// Loads a SPA monitor factory and publishes the <em>device</em> it provides.
    /// </summary>
    /// <param name="ctx">A started context.</param>
    ...
    /// map, which on a client covers only a handful of prefixes and will not find most factories.
    /// </param>
    /// <returns>A handle whose disposal removes the device from the graph.</returns>
    /// <remarks>
    /// What <c>export-spa-device</c> and <c>bluez-session</c> do. The only difference from
    /// <see cref="ExportSpaFactory"/> is which interface is fetched out of the handle: a device
    /// enumerates hardware and creates nodes for what it finds, rather than carrying media itself.
    /// </remarks>
    /// <exception cref="ArgumentNullException"><paramref name="ctx"/> is null.</exception>
    /// <exception cref="ArgumentException"><paramref name="factoryName"/> is null or empty.</exception>
    /// <exception cref="InvalidOperationException">
    ...
    /// </exception>
    public static PipeWireExportedNode ExportSpaDevice(
        PipeWireContext ctx,
        string factoryName,
        IReadOnlyDictionary<string, string>? properties = null,
        string? libraryName = null)
    {
        ...
        var node = new PipeWireExportedNode(ctx, factoryName, SpaDirection.Output);
        node.InitializeFromFactory(factoryName, properties, asDevice: true, libraryName);
        return node;
    }

    private void InitializeFromFactory(
        string factoryName,
        ...
        string? libraryName = null)
    {
        ...
        using (_ctx.Lock())
        {
            ...
            fixed (byte* f = factoryUtf8)
                handle = Native.pw_context_load_spa_handle(_ctx.ContextHandle, (sbyte*)f, &built);

            if (handle is null)
                throw new InvalidOperationException($"the SPA factory '{factoryName}' is not installed.");

            ReadOnlySpan<byte> wanted = asDevice ? NativeConstants.SPA_TYPE_INTERFACE_Device : NativeConstants.SPA_TYPE_INTERFACE_Node;

            void* iface = null;
            int res;
            fixed (byte* type = wanted)
                res = handle->get_interface(handle, (sbyte*)type, &iface);

            if (res < 0 || iface is null)
            {
                throw new InvalidOperationException(
                    $"the SPA factory '{factoryName}' provides no "
                    + $"{(asDevice ? "device" : "node")} interface.");
            }

            _spaHandle = handle;

            fixed (byte* type = wanted)
                _proxy = Native.pw_core_export(core, (sbyte*)type, &built, iface, 0);
        }

        if (_proxy is null)
        {
            Cleanup();
            throw new InvalidOperationException($"pw_core_export refused the factory node '{factoryName}'.");
        }
    }
    ...
    /// <summary>The format this node offers, as given to <see cref="Export"/>.</summary>
    ...
    private static PipeWireExportedNode? From(void* data) =>
        data is null ? null : GCHandle.FromIntPtr((nint)data).Target as PipeWireExportedNode;
    ...
    private int ProduceCycle(spa_io_buffers* io, spa_buffer** buffers, bool[] free, uint bufferCount)
    {
        ...
        if (index == uint.MaxValue) return -NativeConstants.EPIPE;
    ...
    private int EnumeratePortParams(int seq, uint id, uint start, spa_pod* filter)
    {
        ...
        if (length < 0) return -NativeConstants.ENOENT;
